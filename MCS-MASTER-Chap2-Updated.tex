...
\section{Established Techniques for Method Comparison Studies}
...
\end{table}
	...
	the velocities reported in these data sets can not be assumed to be
...
\[ \bar{d} \pm t_{n-1}.\sqrt{S.E(\bar{d})}\]
...
%	The random-effect terms comprise an item-by-replicate interaction term $a_{ir} \sim \mathcal{N}(0,\varsigma^{2})$, a method-by-item interaction term $c_{mi} \sim \mathcal{N}(0,\tau^{2}_{m}),$ and model error terms $\varepsilon_{mir} \sim \mathcal{N}(0,\varphi^{2}_{m}).$ 
	...
	\citet{BXC2008} remarked that for two methods $A$ and $B$, separate values of $g^2_{A}$ and $g^2_{B}$ cannot be estimated, only their average. Hence the assumption that $g_{x}= g_{y}= g$ is necessary. %%---Estimability of Tau
	
	\citet{BXC2008} states a model where the variation between items for method $m$ is captured by $\tau_m$ (our notation $g^2_m$) and the within-item variation by $\sigma_m$.

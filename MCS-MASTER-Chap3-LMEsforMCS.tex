...
The presence of an inter-method bias is the source of disagreement between two methods of measurement that is most easily identified. A formal test for inter-method bias can be implemented by examining the fixed effects of the model. This inter-method bias can be specified as a fixed effect in the LME model. This is common to well known classical linear model methodologies, and interpretation of the results should pose not difficulty to a trained practitioner.
...
Instead, they recommend a fitted mixed effects model to obtain appropriate estimates for the variance of the inter-method bias.

These estimates are based upon variance component estimates derived using LME models. These models estimate variance components for measurements by different methods. 

As their interest mainly lies in extending the Bland-Altman approach, other formal tests are not considered.Their interest lies in generalizing the limits-of-agreement (LOA) method developed by \citet{BA86} to take proper cognizance of the replicate measurements.  Estimation of repeatability is included in this framework.
...
When the design is balanced and there is no ambiguity we can set $n_i=n$. The variation between items for method $m$ is captured by $\sigma_m$ and the within item variation by $\tau_m$.
...
It is not possible to estimate the interaction variance components $\tau^{2}_{1}$ and $\tau^{2}_{2}$ separately, and therefore they are assumed to be equal. The limits of agreement therefore are

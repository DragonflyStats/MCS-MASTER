...
Update formulas for "leave-one-out" estimates typically fail to account for changes in covariance parameters. Update formulae for fixed effects models are available only when the covariance parameters are assumed to be known. As the influence that each item would have on the variance estimate of a method comparison model is crucial part of the model, this disqualifies them.

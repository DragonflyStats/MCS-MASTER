...
\begin{document}
	...
	estimates of the parameters should be obtained. The second step is that either single of multiple data points, specifically outliers,
	should be omitted from the analysis, with the original parameter estimates being updated. This is known as `leave one out \ leave k
	out' analysis. The final step of the procedure is comparing the sets of estimates computed from the entire and reduced data sets
	to determine whether the absence of observations changed the analysis.
	...
	%==================================================================================================== %
	%\subsubsection{Stating the LME Model}
	...
	%
	%
	%\subsubsection{Case Deletion Diagnostics for LME models} %1.6
	%
	...
	\subsection{Christensen et al}         %-Case Deletion section 6.3
	...
	\subsubsection{Cook's Distance}%1.9.2
	...
	\subsection{Computational Limitations for Cook's Distance}
	...
	\subsubsection{Christensen et al}         %-Case Deletion section 6.3
	\citet{Christensen} studied case deletion diagnostics, in particular the analog of Cooks
	distance, for diagnosing influential observations when estimating
	the fixed effect parameters and variance
	components.
	...
	parameters or observations are examined. itet{Christensen} develops \index{case deletion diagnostics} case deletion diagnostics, in particular the equivalent of \index{Cook's distance} Cook's distance, for diagnosing influential observations when estimating the fixed effect parameters and variance components.
	...
	\subsubsection{Terminology for Case Deletion diagnostics} %1.8
	...
	\subsubsection{Deletion Diagnostics}
	...
	%\subsubsection*{3. Case Deletion Diagnostics for LME Data: Cooks Distance, DFBetas}
	In this section we introduce influence analysis and case deletion diagnosics. A full overview of the topic will be provided although there are specific tools that are particularly useful in the case of MCS problems: specifically the Cook's Distance and the DFBeta.
	...
	Note that with k covariates, there will be $k1$ dfbetas (the intercept,$\beta_0$, and one $\beta$ for each covariate). When the model is specified without an intercept term, as in the last chapter, there is a set of DFBETAs corresponding to each measurement method.
	...
	\subsubsection{Application of DFBETAs in MCS Analysis}
	...
	\subsection{Application of DFBETAs in MCS Analysis}
	...
	\subsubsection{Model Diagnostics for Roy's Models}
	...
	\subsection{Case Deletion Diagnostics for Variance Ratios}
